-- Blocking System Implementation (Fixed Version)
-- This script creates the necessary tables and policies for user blocking functionality
-- Fixed to work with existing table structures
...
-- First, let's check what the actual seller column is called
-- The products table might use 'seller_id' instead of 'user_id'
DROP POLICY IF EXISTS "Users can view all products" ON products;
CREATE POLICY "Users can view all products" ON products
    FOR SELECT USING (
        NOT EXISTS (
            SELECT 1 FROM blocked_users 
            WHERE (blocker_id = auth.uid() AND blocked_id = products.seller_id) OR
                  (blocked_id = auth.uid() AND blocker_id = products.seller_id)
        )
    );
